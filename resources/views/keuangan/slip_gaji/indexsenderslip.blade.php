            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive pt-3">
                        <form action="{{ route('slip_gaji.IndexSendSlip') }}" method="GET">
                            <div class="row">
                                <div class="col-md-4">
                        ...
                        <form action="{{ route('slip_gaji.SendAllSlip') }}" method="POST">
                            @csrf
                                    ...
                                    @foreach($employees as $key => $employee)
                                        <tr>
                                            <td>
                                                ...
                                                <input type="hidden" name="employee_data[{{ $employee->id }}][nama]" value="{{ $employee->nama_lengkap }}">
                                                <input type="hidden" name="employee_data[{{ $employee->id }}][telepon]" value="{{ $employee->telepon }}">
                                            </td>
                                            <td>{{ $key + 1 }}</td>
                                            ...
                                            <td>
                                                @if($SlipPenggajians->where('employee_id', $employee->id)->isEmpty())
                                                    Belum Buat Slip
                                                @else
                                                    <button type="button" class="btn btn-warning btn-sm rincian-gaji-btn" data-id="{{ $employee->id }}">Rincian Gaji</button>
                                                @endif
                                            </td>
                                            <td>
                                                @if($SlipPenggajians->where('employee_id', $employee->id)->isEmpty())
                                            ...
                                            <td>
                                                @if($SlipPenggajians->where('employee_id', $employee->id)->isNotEmpty())
                                                    <a href="{{ route('slip_gaji.SendSlip',  ['id' => $employee->id, 'bulan' => $bulan, 'tahun' => $tahun]) }}" class="btn btn-primary btn-sm">Kirim Slip WA</a>
                                                @endif
                                            </td>
                                        </tr>
                                    @endforeach
                            ...
                            <button type="submit" class="btn btn-primary">Kirim Slip Gaji</button>
                        </form>
