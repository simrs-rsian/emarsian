...
<div class="col-12 grid-margin">
    <div class="card">
        <div class="card-body">
            <p class="card-description"><h3>Data Riwayat Cuti Kode {{ $cuti->kode_cuti }}</h3></p>
            <div class="row">
                <div class="col-md-12">
                    <div class="mb-3">
                        <a href="{{ route('perizinan.riwayat.cuti.index', ['employee_id' => $cuti->id_employee]) }}" class="btn btn-danger btn-sm">Kembali</a>
                        <!-- tombol print -->
                        <a href="{{ route('perizinan.riwayat.cuti.download', ['kodeCuti' => $cuti->kode_cuti]) }}" class="btn btn-success btn-sm" target="_blank">Download File</a>
                    </div>
                    <br>
                            ...
                            <tr style="height: 80px;">
                                <td>
                                    @if(isset($cuti->ttd_mengetahui))
                                        ...
                                        <form action="{{ route('perizinan.riwayat.cuti.nullTtd', ['kodeCuti' => $cuti->kode_cuti, 'ttd' => 'mengetahui']) }}" method="POST" id="pengantar-form">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="kode_cuti" value="{{ $cuti->kode_cuti }}">
                                            <input type="hidden" name="ttd_mengetahui" value="">
                                            <input type="hidden" name="mengetahui" value="">
                                            <button type="submit" class="btn btn-danger" title="Hapus Tanda tangan">
                                ...
                                <td>
                                    @if(isset($cuti->ttd_menyetujui))
                                        ...
                                        <form action="{{ route('perizinan.riwayat.cuti.nullTtd', ['kodeCuti' => $cuti->kode_cuti ]) }}" method="POST" id="menyetujui-form">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="kode_cuti" value="{{ $cuti->kode_cuti }}">
                                ...
                                <td>
                                    @if(isset($cuti->ttd_karyawan_pengganti))
                                        ...
                                        <form action="{{ route('perizinan.riwayat.cuti.nullTtd', ['kodeCuti' => $cuti->kode_cuti ]) }}" method="POST" id="pengganti-form">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="kode_cuti" value="{{ $cuti->kode_cuti }}">
                                ...
                                <td>
                                    @if(isset($cuti->ttd_karyawan_pemohon))
                                        ...
                                        <form action="{{ route('perizinan.riwayat.cuti.nullTtd', ['kodeCuti' => $cuti->kode_cuti ]) }}" method="POST" id="pemohon-form">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="kode_cuti" value="{{ $cuti->kode_cuti }}">
                                ...
                                <td>
                                    @if(isset($cuti->ttd_pencatat))
                                        ...
                                        <form action="{{ route('perizinan.riwayat.cuti.nullTtd', ['kodeCuti' => $cuti->kode_cuti ]) }}" method="POST" id="pencatat-form">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="kode_cuti" value="{{ $cuti->kode_cuti }}">
                                            <input type="hidden" name="ttd_pencatat" value="">
                                            <input type="hidden" name="pencatat" value="">
                                            <button type="submit" class="btn btn-danger" title="Hapus Tanda tangan">
                                                <i class="bi bi-trash"></i>
                                            </button>
                                        </form>
...
<script>
    ...
    document.getElementById('save-canvas').addEventListener('click', function() {
        ...
        if (modalTitle === "TTD Mengetahui Ka. Subbag SDI") {
            route = '{{  route("perizinan.riwayat.cuti.ttdMengetahui") }}';
            data.mengetahui = inputName;
        } else if (modalTitle === "TTD Menyetujui Atasan Langsung") {
            route = '{{  route("perizinan.riwayat.cuti.ttdMenyetujui") }}';
            data.menyetujui = inputName;
        } else if (modalTitle === "TTD Pencatat") {
            route = '{{  route("perizinan.riwayat.cuti.ttdPencatat") }}';
            data.pencatat = inputName;
        }else if (modalTitle === "TTD Karyawan Pemohon") {
            route = '{{  route("perizinan.riwayat.cuti.ttdPemohon") }}';
        }else if (modalTitle === "TTD Karyawan Pengganti") {
            route = '{{  route("perizinan.riwayat.cuti.ttdPengganti") }}';
        }
