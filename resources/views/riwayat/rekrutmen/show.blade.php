    ...
    <div class="card">
        <div class="card-body">
          <!-- buat session edit berhasil atau error -->
            ...
            <p class="card-description"><h3>Data Rekrutmen {{ $employee->nama_lengkap }} (NIP : {{ $employee->nip_karyawan }})</h3></p>
            <div class="row">
                <div class="col-md-12">
                    <div class="mb-3">
                        <a href="{{ route('employee.show', $employee->id) }}" class="btn btn-danger btn-sm">Kembali</a>
                        <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#createriwayatRekrutmenModal">Tambah Data Rekrutmen</button>
                    </div>
                    <br>
                    <div class="table-responsive" data-simplebar>
                        <table class="table table-borderless align-middle text-nowrap" id="dataTable">
                            ...
                            <tbody>
                                @foreach($riwayatRekrutmens as $key => $riwayatRekrutmen)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $riwayatRekrutmen->nama_riwayat }}</td>
                                        <td>{{ $riwayatRekrutmen->tanggal_riwayat }}</td>
                                        <td>
                                            @if($riwayatRekrutmen->dokumen)
                                                <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#viewDokumenLainModal{{ $riwayatRekrutmen->id }}">
                                                    Tampilalkan Dokumen
                                                </button>
                                            @else
                                                -
                                        ...
                                        <td>
                                            <a href="{{ route('riwayat_rekrutmen.edit', $riwayatRekrutmen->id) }}" class="btn btn-warning btn-sm">Edit</a>
                                            <form action="{{ route('riwayat_rekrutmen.destroy', $riwayatRekrutmen->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
            ...
            @foreach($riwayatRekrutmens as $key => $riwayatRekrutmen)
                ...
                <div class="modal fade" id="viewDokumenLainModal{{ $riwayatRekrutmen->id }}" tabindex="-1" aria-labelledby="viewDokumenLainModalLabel{{ $riwayatRekrutmen->id }}" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="viewDokumenModalJabatanLabel{{ $riwayatRekrutmen->id }}">Dokumen Riwayat Lain</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body text-center">
                                @if($riwayatRekrutmen->dokumen)
                                    @php
                                        $extension = pathinfo($riwayatRekrutmen->dokumen, PATHINFO_EXTENSION);
                                    @endphp
                                    
                                    @if(in_array($extension, ['jpg', 'jpeg', 'png', 'gif']))
                                        <img src="{{ url($riwayatRekrutmen->dokumen) }}" alt="Dokumen" class="img-fluid">
                                    @elseif($extension == 'pdf')
                                        <iframe src="{{ url($riwayatRekrutmen->dokumen) }}" width="100%" height="500px"></iframe>
                                    @else
            ...
            <div class="modal fade" id="createriwayatRekrutmenModal" tabindex="-1" aria-labelledby="createriwayatRekrutmenModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="createriwayatRekrutmenModalLabel">Tambah Data Rekrutmen</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>

                        <form action="{{ route('riwayat_rekrutmen.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
